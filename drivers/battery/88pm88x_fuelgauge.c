			...
			if (info->bat_params.volt <= temp_power_off_th) {
				if (power_off_cnt > 4) {
					if ((info->bat_params.temp < 0) && (power_off_cnt < 20)) {
						dev_info(info->dev, "%s: hit power off cnt at low temp = %d\n",
							__func__, power_off_cnt);
						power_off_cnt++;
					} else {
				 ...
				 int *initial_soc, int *initial_cycles)
{
	...
	if (info->chip->powerup & 0x2) {
		...
		if (saved_is_valid < 0) {
			...
			info->ocv_is_realiable = false;
		} else if ((soc_from_vbat_active - soc_from_vbat_slp) > 3) {
			*initial_soc = soc_from_vbat_active;
		} else {
			if (soc_from_saved < 5)
				remove_th = 8;
			else if (soc_from_saved < 15)
				remove_th = 13;
			else if (soc_from_saved < 80)
				remove_th = 25;
			else if (soc_from_saved < 95)
				remove_th = 8;
			else if (soc_from_saved < 99)
				remove_th = 3;
			else
				remove_th = 1;

			if (!!(abs(soc_from_vbat_active - soc_from_saved) > remove_th))
				*initial_soc = soc_from_vbat_active;
			else
				*initial_soc = soc_from_saved;

			dev_info(info->dev,
				"%s: soc_from_vbat_active = %d, soc_from_saved = %d, remove = %d\n",
				__func__, soc_from_vbat_active, soc_from_saved, remove_th);
		}
		*initial_cycles = cycles_from_saved;
