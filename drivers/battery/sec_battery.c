...
static enum extcon_cable_name previous_cable_type = CABLE_UNKNOWN;
				...
				bool enable)
{
	...
	val.intval = battery->pdata->always_enable ? battery->cable_type :
		enable ? battery->cable_type : POWER_SUPPLY_TYPE_BATTERY;
...
#ifndef CONFIG_FUELGAUGE_88PM88X
		...
		struct sec_battery_info *battery, int health)
{
	if (battery->health != health) {
		...
		case POWER_SUPPLY_HEALTH_UNDERVOLTAGE:
			...
			sec_bat_set_charging_status(battery,
					POWER_SUPPLY_STATUS_NOT_CHARGING);
			/* Remove charger disable because we are unable to recover 
			   charging in Marvell 88PM88X devices */
#if !defined(CONFIG_CHARGER_88PM88X)
			sec_bat_set_charge(battery, false);
#endif
...
#if defined(CONFIG_SEC_GPIO_DVS)
static void sec_init_dvs_work(struct work_struct *work)
{
				...
				enum extcon_cable_name attached_dev)
{
	...
	case CABLE_JIG_UART:
		...
		if (battery->vbus_present) {
			...
			pr_info("%s: JIG + VBUS present\n", __func__);
		} else {
			pr_info("%s: JIG VBUS NOT present\n", __func__);
			current_cable_type = POWER_SUPPLY_TYPE_BATTERY;
		}
		break;
	case CABLE_DESKTOP_DOCK:
		if (battery->vbus_present) {
			current_cable_type = POWER_SUPPLY_TYPE_MAINS;
			pr_info("%s: DESKDOCK + VBUS present\n", __func__);
		} else {
			pr_info("%s: DESKDOCK VBUS NOT present\n", __func__);
			current_cable_type = POWER_SUPPLY_TYPE_BATTERY;
		}
		break;
	case CABLE_TA:
	...
	previous_cable_type = attached_dev;
	return current_cable_type;
}

static int batt_process_cable_type(int cable_type,
		struct sec_battery_info *battery)
{
	if (cable_type < 0) {
		...
		unsigned long action, void *data)
{
	...
	batt_process_cable_type(cable_type, battery);
		...
		unsigned long action, void *data)
{
	...
	switch (action) {
	case CABLE_VBUS_NONE:
		cmd = "VBUS DETACH";
		battery->vbus_present  = false;
		cable_type = POWER_SUPPLY_TYPE_BATTERY;
		break;
	case CABLE_VBUS_OCCURRENCE:
		cmd = "VBUS ATTACH";
		battery->vbus_present  = true;
		pr_info("%s: VBUS attached: Maintain the same cable type\n",
			 __func__);
		cable_type = sec_bat_cable_check(battery, previous_cable_type);
		break;
	default:
		cmd = "ERROR";
		cable_type = -1;
	...
	for (i=0; i < CABLE_END; i++) {
		...
		if (extcon_get_cable_state_(battery->extcon_cable_list[i].extcon_nb.edev, i)) {
			...
			pr_info("%s: %s(wire_status = %d) attached from extcon\n", __func__,
				extsec_cable[i], battery->wire_status);
		}
	}

	ret = vbus_register_notify(&battery->vbus_nb);
	battery->vbus_nb.notifier_call = vbus_handle_notification;
	if (ret < 0)
